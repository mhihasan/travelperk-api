...
class TestGetOrder:
    ...
    async def test_gets_order(self, db_session: AsyncSession):
        ...
        created_order = (await order_service.list_orders(db_session))[0]

        order = await order_service.get_order(db_session, created_order.id)
...
class TestUpdateOrder:
    ...
    async def test_updates_order(self, db_session: AsyncSession):
        ...
        updated_order = await order_service.get_order(db_session, created_order.id)
